...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-11 | V25.8-R4 live Shadow wiring | pseudo-hit read-only fan-out / V25 Combat / dry-run Bridge | The real launcher can now start independent V25 Visualization, Combat and Game Bridge processes from an explicit disposable-profile switch. Combat tails `pseudo_hit_all.jsonl`; Legacy-only rows are labeled compatibility observations with `algorithm_evaluated=false`. | No Event worker, tracking, pseudo-hit generation, final-hit authority or Legacy Game Bridge change. All new outputs are run-namespaced, non-official and network-disabled. | Remote candidate 1 `v25_8_r4_20260711_131628_display1_keepstdin` passed with all three processes/statuses sampled 116 times, 115 connected input samples, zero network attempts and verified restore. | The unattended scene produced zero pseudo-hit rows, so remote event processing was idle. Functional hit-path acceptance remains a deterministic R5 replay/fault gate, not a field-effectiveness claim. |
| 2026-07-11 | V25.8-R3 BEV/polygon/disk gate | V25 sequence map consumption / Global BEV display | Candidate Global BEV consumed fresh session-scoped Event-to-MVS rings from tmpfs, retained GUI-thread OpenGL ownership, and published output on accumulated monotonic deadlines. Polygon validity is now explicit in unified status. | Display/status only for the Event side. Raw Event capture, tracking, pseudo hit, final hit, judgement sync threshold, and production Event rings are unchanged. | Remote candidate 3 `v25_8_r3_20260711_120643_display1_keepstdin` passed: BEV render 30.329fps, output 12.762fps, zero MVS late-drop, patch restore verified. | The unattended scene had no visible sparse Event points, so pixel-level Event/MVS overlay alignment is still unvalidated. Candidate rings were tmpfs-only and the production profile was not changed. |
