...
def main() -> int:
    ...
    for s in summaries:
        print(f"[{s['camera']}] mvs={s['mvs_rows']} trigger={s['trigger_rows']} human={s['human_rows']} coverage={s.get('human_coverage_pct', 0.0)}% with_person={s.get('human_with_person_pct', 0.0)}% -> {s['out_path']}")
