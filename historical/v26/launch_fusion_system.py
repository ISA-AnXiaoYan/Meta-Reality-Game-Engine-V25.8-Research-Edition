...
def _patch_event_runtime_cpu_options(event_cfg_path: Path, project_root: Path, args, cams: List[str]) -> None:
    ...
    try:
        ...
        shared['event_track_polygon_gate_mode'] = str(getattr(args, 'event_track_polygon_gate_mode', 'off') or 'off')
        shared['event_track_polygon_gate_cameras'] = str(getattr(args, 'event_track_polygon_gate_cameras', '') or '')
        shared['event_track_polygon_gate_hold_enable'] = bool(getattr(args, 'event_track_polygon_gate_hold_enable', True))
        shared['event_track_polygon_gate_hold_ms'] = float(getattr(args, 'event_track_polygon_gate_hold_ms', 30.0) or 0.0)
        shared['event_track_polygon_gate_hold_max_sync_gap'] = int(getattr(args, 'event_track_polygon_gate_hold_max_sync_gap', 2) or 0)
        shared['event_track_polygon_gate_dilate_px'] = int(getattr(args, 'event_track_polygon_gate_dilate_px', 0) or 0)
        shared['event_track_polygon_gate_log_jsonl'] = str(getattr(args, 'event_track_polygon_gate_log_jsonl', '') or '')
        shared['event_track_polygon_gate_log_max_fps'] = float(getattr(args, 'event_track_polygon_gate_log_max_fps', 5.0) or 0.0)
        shared['event_business_id_v1_shadow_enable'] = bool(getattr(args, 'event_business_id_v1_shadow_enable', False))
        shared['event_business_id_v1_shadow_cameras'] = str(getattr(args, 'event_business_id_v1_shadow_cameras', '') or '')
        shared['event_business_id_v1_confirm_min_observations'] = int(getattr(args, 'event_business_id_v1_confirm_min_observations', 4) or 4)
        shared['event_business_id_v1_confirm_min_displacement_px'] = float(getattr(args, 'event_business_id_v1_confirm_min_displacement_px', 20.0) or 0.0)
        shared['event_business_id_v1_confirm_min_speed_px_ms'] = float(getattr(args, 'event_business_id_v1_confirm_min_speed_px_ms', 0.32) or 0.0)
        shared['event_business_id_v1_assoc_radius_px'] = float(getattr(args, 'event_business_id_v1_assoc_radius_px', 28.0) or 0.0)
        shared['event_business_id_v1_max_gap_ms'] = float(getattr(args, 'event_business_id_v1_max_gap_ms', 120.0) or 0.0)
        shared['event_business_id_v1_collision_window_ms'] = float(getattr(args, 'event_business_id_v1_collision_window_ms', 260.0) or 0.0)
        shared['event_business_id_v1_collision_max_gap_px'] = float(getattr(args, 'event_business_id_v1_collision_max_gap_px', 95.0) or 0.0)
        shared['event_business_id_v1_shadow_log_jsonl'] = str(getattr(args, 'event_business_id_v1_shadow_log_jsonl', '') or '')
        shared['event_business_id_v1_shadow_log_max_fps'] = float(getattr(args, 'event_business_id_v1_shadow_log_max_fps', 5.0) or 0.0)
        shared['event_business_id_v3_mode'] = str(getattr(args, 'event_business_id_v3_mode', 'off') or 'off')
        shared['event_business_id_v3_cameras'] = str(getattr(args, 'event_business_id_v3_cameras', '') or '')
        shared['event_business_id_v3_size_warmup_observations'] = int(getattr(args, 'event_business_id_v3_size_warmup_observations', 6) or 6)
        shared['event_business_id_v3_max_size_ratio'] = float(getattr(args, 'event_business_id_v3_max_size_ratio', 3.0) or 0.0)
        shared['event_business_id_v3_size_outlier_consecutive'] = int(getattr(args, 'event_business_id_v3_size_outlier_consecutive', 3) or 3)
        shared['event_business_id_v3_log_jsonl'] = str(getattr(args, 'event_business_id_v3_log_jsonl', '') or '')
        shared['event_business_id_v3_log_max_fps'] = float(getattr(args, 'event_business_id_v3_log_max_fps', 5.0) or 0.0)
...
def main() -> int:
    ...
    ap.add_argument('--v25-live-shadow-enable', action='store_true', default=False, help='Launch V25 Visualization, Combat and Game Bridge as namespaced non-official services.')
    ap.add_argument('--disable-v25-live-shadow', dest='v25_live_shadow_enable', action='store_false')
    ap.add_argument('--v25-live-shadow-root', default='v25_shadow/live')
    ap.add_argument('--v25-live-shadow-session-id', default='')
    ap.add_argument('--v25-live-shadow-poll-ms', type=float, default=100.0)
    ap.add_argument('--v25-live-shadow-status-interval-ms', type=float, default=500.0)
    ap.add_argument('--v25-live-shadow-max-input-age-ms', type=float, default=2500.0)
    ap.add_argument('--v25-live-shadow-visualization-cpus', default='')
    ap.add_argument('--v25-live-shadow-combat-cpus', default='')
    ap.add_argument('--v25-live-shadow-game-bridge-cpus', default='')
    ...
    ap.add_argument('--mvs-trigger-preamble-guard-ms', type=float, default=-1.0, help='Optional coded-preamble guard forwarded to the trigger coordinator; <=0 keeps coordinator default.')
    ap.add_argument('--mvs-trigger-preamble-ack-timeout-ms', type=float, default=-1.0, help='Optional coded-preamble ack timeout forwarded to the trigger coordinator; <=0 keeps coordinator default.')
    ap.add_argument('--mvs-trigger-preamble-max-attempts', type=int, default=-1, help='Optional coded-preamble attempt count forwarded to the trigger coordinator; <=0 keeps coordinator default.')
    ...
    ap.add_argument('--mvs-worker-late-drop-ms', type=float, default=-1.0, help='Forwarded to split MVS workers. Drop ticks only when received later than this budget; <=0 keeps worker/config default.')
    ...
    ap.add_argument('--event-track-polygon-gate-mode', choices=['off', 'shadow', 'active'], default='off')
    ap.add_argument('--event-track-polygon-gate-cameras', default='')
    ap.add_argument('--event-track-polygon-gate-hold-enable', action='store_true', default=True)
    ap.add_argument('--disable-event-track-polygon-gate-hold', dest='event_track_polygon_gate_hold_enable', action='store_false')
    ap.add_argument('--event-track-polygon-gate-hold-ms', type=float, default=30.0)
    ap.add_argument('--event-track-polygon-gate-hold-max-sync-gap', type=int, default=2)
    ap.add_argument('--event-track-polygon-gate-dilate-px', type=int, default=0)
    ap.add_argument('--event-track-polygon-gate-log-jsonl', default='')
    ap.add_argument('--event-track-polygon-gate-log-max-fps', type=float, default=5.0)
    ap.add_argument('--event-business-id-v1-shadow-enable', action='store_true', default=False)
    ap.add_argument('--event-business-id-v1-shadow-cameras', default='')
    ap.add_argument('--event-business-id-v1-confirm-min-observations', type=int, default=4)
    ap.add_argument('--event-business-id-v1-confirm-min-displacement-px', type=float, default=20.0)
    ap.add_argument('--event-business-id-v1-confirm-min-speed-px-ms', type=float, default=0.32)
    ap.add_argument('--event-business-id-v1-assoc-radius-px', type=float, default=28.0)
    ap.add_argument('--event-business-id-v1-max-gap-ms', type=float, default=120.0)
    ap.add_argument('--event-business-id-v1-collision-window-ms', type=float, default=260.0)
    ap.add_argument('--event-business-id-v1-collision-max-gap-px', type=float, default=95.0)
    ap.add_argument('--event-business-id-v1-shadow-log-jsonl', default='')
    ap.add_argument('--event-business-id-v1-shadow-log-max-fps', type=float, default=5.0)
    ap.add_argument('--event-business-id-v3-mode', choices=['off', 'shadow'], default='off')
    ap.add_argument('--event-business-id-v3-cameras', default='')
    ap.add_argument('--event-business-id-v3-size-warmup-observations', type=int, default=6)
    ap.add_argument('--event-business-id-v3-max-size-ratio', type=float, default=3.0)
    ap.add_argument('--event-business-id-v3-size-outlier-consecutive', type=int, default=3)
    ap.add_argument('--event-business-id-v3-log-jsonl', default='')
    ap.add_argument('--event-business-id-v3-log-max-fps', type=float, default=5.0)
    ...
    try:
        ...
        if not args.skip_mvs:
            ...
            if mvs_source_mode == 'file_replay':
                ...
                for idx, cid in enumerate(cams):
                    ...
                    if float(getattr(args, 'mvs_worker_late_drop_ms', -1.0) or -1.0) > 0.0:
                        worker_cmd += ['--late-drop-ms', str(float(args.mvs_worker_late_drop_ms))]
                ...
                if float(getattr(args, 'mvs_trigger_preamble_guard_ms', -1.0) or -1.0) > 0.0:
                    coord_cmd += ['--preamble-guard-ms', str(float(args.mvs_trigger_preamble_guard_ms))]
                if float(getattr(args, 'mvs_trigger_preamble_ack_timeout_ms', -1.0) or -1.0) > 0.0:
                    coord_cmd += ['--preamble-ack-timeout-ms', str(float(args.mvs_trigger_preamble_ack_timeout_ms))]
                if int(getattr(args, 'mvs_trigger_preamble_max_attempts', -1) or -1) > 0:
                    coord_cmd += ['--preamble-max-attempts', str(int(args.mvs_trigger_preamble_max_attempts))]
        ...
        if bool(getattr(args, 'game_event_bridge_enable', False)):
            ...
            procs.append(p); p.start()
        if bool(getattr(args, 'v25_live_shadow_enable', False)):
            live_root = Path(sync_abs_path(str(args.v25_live_shadow_root))).resolve()
            live_session = str(args.v25_live_shadow_session_id or f"launcher-{os.getpid()}-{int(time.time())}")
            live_poll_ms = max(20.0, float(args.v25_live_shadow_poll_ms))
            live_status_ms = max(100.0, float(args.v25_live_shadow_status_interval_ms))
            live_max_age_ms = max(100.0, float(args.v25_live_shadow_max_input_age_ms))
            visualization_root = live_root / 'visualization'
            combat_root = live_root / 'combat'
            game_bridge_root = live_root / 'game_bridge'
            visualization_cmd = [
                py, '-m', 'mrg_visualization.live_shadow',
                '--input-json', sync_abs_path(str(args.global_person_id_latest_json)),
                '--output-root', str(visualization_root),
                '--session-id', live_session,
                '--poll-ms', str(live_poll_ms),
                '--status-interval-ms', str(live_status_ms),
                '--max-input-age-ms', str(live_max_age_ms),
            ]
            combat_cmd = [
                py, '-m', 'mrg_combat_engine.live_shadow',
                '--input-jsonl', sync_abs_path('pseudo_hit_all.jsonl'),
                '--health-json', sync_abs_path('hit_judge_status.json'),
                '--output-root', str(combat_root),
                '--session-id', live_session,
                '--poll-ms', str(live_poll_ms),
                '--status-interval-ms', str(live_status_ms),
                '--max-health-age-ms', str(live_max_age_ms),
            ]
            game_bridge_cmd = [
                py, '-m', 'mrg_game_bridge.live_shadow',
                '--input-jsonl', str(combat_root / 'decisions.jsonl'),
                '--health-json', str(combat_root / 'status.json'),
                '--output-root', str(game_bridge_root),
                '--session-id', live_session,
                '--poll-ms', str(live_poll_ms),
                '--status-interval-ms', str(live_status_ms),
                '--max-health-age-ms', str(live_max_age_ms),
            ...
            live_commands = (
                ('V25_VISUALIZATION_SHADOW', visualization_cmd, str(args.v25_live_shadow_visualization_cpus or '')),
                ('V25_COMBAT_SHADOW', combat_cmd, str(args.v25_live_shadow_combat_cpus or '')),
                ('V25_GAME_BRIDGE_SHADOW', game_bridge_cmd, str(args.v25_live_shadow_game_bridge_cpus or '')),
            )
            log_launcher(f"V25 live Shadow enabled session={live_session} root={live_root} official_result=false")
            for proc_name, command, cpus in live_commands:
                p = make_proc(proc_name, _taskset_cmd(cpus, command), critical=False)
                procs.append(p); p.start()
        if args.gl_delay > 0:
        ...
        if not args.skip_gl:
            ...
            if args.preview_backend == 'pyopengl-cuda':
                gl_cmd = [
                    ...
                    '--sync-root', str((project_root / 'sync_ipc').resolve()),
                    '--v25-live-shadow-root', sync_abs_path(str(args.v25_live_shadow_root)),
