...
- 当前批次没有 person 时，worker 状态为 `valid_empty`，属于合法空输出，不再导致 merge 误报失败。
- 只要存在 person 但 polygon 缺失或非法，worker/merge 状态必须 fail closed，不能用历史 ratio 掩盖。
- 运行状态新增稳定字段：`polygon_contract_state`、`polygon_output_enabled`、`published_persons_without_polygon`。
- unified status 必须传播这些字段；下游不得仅用 `published_persons_with_polygon_ratio` 判断无人场景。
- R3 远端候选 3 的 merge 为 `valid_complete`、缺 polygon person 为 0；Global YOLO 吞吐仍按 D056 仅监视，不作为本工作包性能优化目标。
