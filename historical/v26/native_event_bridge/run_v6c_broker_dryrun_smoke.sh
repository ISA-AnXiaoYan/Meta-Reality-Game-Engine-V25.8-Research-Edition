...
DURATION_S="${1:-60}"
RUN_ID="v6c_broker_dryrun_$(date +%Y%m%d_%H%M%S)"
...
pgrep -af 'hal_event_fifo_broker|hal_event_pipe_bridge|metavision_spatter_tracking_linefilter_tsf1_autosender_backfill' > "$OUT/processes_after.txt" 2>&1 || true
