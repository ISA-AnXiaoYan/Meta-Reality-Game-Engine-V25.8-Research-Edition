...
class _SimpleRateLimiter:
    ...
    def allow(self):
        if self.min_interval_s <= 0.0:
            ...
            return True
        now = time.monotonic()
        if self.last_s <= 0.0 or (now - self.last_s) >= self.min_interval_s:
            self.last_s = now
            self.allowed += 1
            return True
        self.skipped += 1
...
def main():
    ...
    try:
        ...
        for evs in mv_iterator:
            ...
            if bool(getattr(args, 'event_empty_slice_fastpath_enable', True)) and (raw_n <= 0 or detection_n <= 0):
                iter_idx += 1
                _update_event_worker_status(
                    state='idle',
                    active_reason='empty_slice' if raw_n <= 0 else 'mask_filtered_empty',
