...
from global_yolo_polygon_contract import summarize_polygon_contract
...
class GlobalYoloMergePublisher:
    ...
    def _merge_status(self, state: str = "running", error: str = "") -> Dict[str, Any]:
        ...
        for idx, status in enumerate(self._last_status_by_worker):
            ...
            worker_polygon_output_enabled = bool(
                status.get("polygon_output_enabled", False) if isinstance(status, dict) else False
            ) or bool(diagnostics.get("include_mask_polygons", False))
            ...
            workers.append({
                ...
                "polygon_output_enabled": bool(worker_polygon_output_enabled),
                "polygon_contract_state": str(status.get("polygon_contract_state", "") if isinstance(status, dict) else ""),
        ...
        records_with_persons = sum(
            _safe_int((w.get("stats") or {}).get("published_records_with_persons"), 0)
            for w in workers
        )
        records_with_polygon = sum(
            _safe_int((w.get("stats") or {}).get("published_records_with_polygon"), 0)
            for w in workers
        )
        record_polygon_ratio = 1.0 if records_with_persons == 0 else records_with_polygon / float(records_with_persons)
        persons_total = sum(
            _safe_int((w.get("stats") or {}).get("published_persons_total"), 0)
            for w in workers
        )
        persons_with_polygon = sum(
            _safe_int((w.get("stats") or {}).get("published_persons_with_polygon"), 0)
            for w in workers
        )
        polygon_output_enabled = bool(workers) and all(bool(w.get("polygon_output_enabled")) for w in workers)
        polygon_contract = summarize_polygon_contract(
            persons_total=persons_total,
            persons_with_polygon=persons_with_polygon,
            polygon_output_enabled=polygon_output_enabled,
            dropped_batches=dropped_batches,
        )
        polygon_ratio = float(polygon_contract["published_persons_with_polygon_ratio"])
