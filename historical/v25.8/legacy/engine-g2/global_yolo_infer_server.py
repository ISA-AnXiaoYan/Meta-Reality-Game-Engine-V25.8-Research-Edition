...
class GlobalYoloServer:
    ...
    def _update_polygon_contract_stats(self, records: List[Dict[str, Any]]) -> None:
        ...
        for rec in records:
            ...
            for person in persons:
                if not isinstance(person, dict):
                    continue
                persons_total += 1
                polys = person.get("mask_polygons") or person.get("mask_polygon") or person.get("polygons")
                has_polygon = isinstance(polys, list) and len(polys) > 0
                if has_polygon:
        ...
        records_den = max(1, int(self.stats.get("published_records_with_persons", 0) or 0))
        persons_den = max(1, int(self.stats.get("published_persons_total", 0) or 0))
        self.stats["published_records_with_polygon_ratio"] = round(float(self.stats.get("published_records_with_polygon", 0) or 0) / records_den, 4)
        self.stats["published_persons_with_polygon_ratio"] = round(float(self.stats.get("published_persons_with_polygon", 0) or 0) / persons_den, 4)
    ...
    def _write_status(self, state: str, last_batch: Optional[Dict[str, Any]] = None, error: str = "") -> float:
        ...
        polygon_ratio = float(self.stats.get("published_persons_with_polygon_ratio", 1.0) or 0.0)
        polygon_contract_ok = (
            not bool(self.args.include_mask_polygons)
            or (
                int(self.stats.get("postprocess_dropped_batches", 0) or 0) == 0
                and (
                    int(self.stats.get("published_persons_total", 0) or 0) == 0
                    or polygon_ratio >= 0.90
                )
            )
        )
