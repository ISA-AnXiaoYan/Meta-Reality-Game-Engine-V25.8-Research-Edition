...
def _prepare_view_mode_configs(root: Path, log_dir: Path, event_config: str, human_config: str, view_mode: str) -> tuple[Path, Path]:
    event_path = _resolve_arg_path(root, event_config)
    human_path = _resolve_arg_path(root, human_config)
    mode = str(view_mode or 'config').strip().lower()
    if mode == 'config':
        return event_path, human_path

    event_cfg = _load_json(event_path)
    human_cfg = _load_json(human_path)
    shared = event_cfg.setdefault('shared_options', {})
    overlay = human_cfg.setdefault('event_overlay', {})
    overlay['_save_path'] = str(human_path)

    if mode == 'separate':
        shared['headless'] = False
        shared['enable_event_overlay_pub'] = False
        overlay['enabled'] = False
    elif mode == 'fused':
        shared['headless'] = True
        shared['enable_event_overlay_pub'] = True
        shared.setdefault('event_overlay_pub_name', 'event_overlay_latest_{camera}')
        shared.setdefault('event_overlay_pub_scale', 1.0)
        shared.setdefault('event_overlay_pub_mode', 'overlay')
    ...
    event_out = log_dir / f'event_config_{mode}.json'
    human_out = log_dir / f'human_config_{mode}.json'
    _write_json(event_out, event_cfg)
    _write_json(human_out, human_cfg)
    print(f'[launcher] view_mode={mode}: event_config={event_out}', flush=True)
    print(f'[launcher] view_mode={mode}: human_config={human_out}', flush=True)
    return event_out, human_out
...
def main() -> int:
    ...
    ap.add_argument('--view-mode', choices=['config', 'separate', 'fused', 'both'], default='config',
                    help='显示模式：config=按 JSON；separate=事件/MVS 分开；fused=只显示 MVS 融合窗口；both=分开窗口和融合窗口都显示')
    ...
    mkdirp(sync_ipc)
    mkdirp(log_dir)
    event_config_for_launch, human_config_for_launch = _prepare_view_mode_configs(
        root, log_dir, args.event_config, args.human_config, args.view_mode
    )
    ...
    try:
        if not args.human_only:
            event_cmd = [args.python, str(root / args.event_launcher), '--config', str(event_config_for_launch)]
        ...
        if not args.event_only:
            if args.human_only:
                wait_ready(ready_files, timeout_s=float(args.ready_timeout_s))
            human_cmd = [args.python, str(root / args.human_script), '--config', str(human_config_for_launch)]
