...
def assign_local_positions(persons: List[PersonInstance], projector: Optional[LocalMapProjector], args: argparse.Namespace) -> None:
    for p in persons:
        ...
        if projector is not None and projector.enabled:
            p.local_xy = projector.project(foot)
...
def draw_instances(frame_bgr: np.ndarray, persons: List[PersonInstance], args: argparse.Namespace, fps_text: str = "") -> np.ndarray:
    ...
    overlay = vis.copy()
    for p in persons:
        color = make_color(p.instance_id - 1)
        if bool(args.draw_mask):
            overlay[p.mask > 0] = color
        if bool(args.draw_contour):
            cv2.drawContours(vis, p.contours, -1, color, int(args.contour_thickness), cv2.LINE_AA)
        if bool(args.draw_box):
            x1, y1, x2, y2 = p.bbox_xyxy
            cv2.rectangle(vis, (x1, y1), (x2, y2), color, 1)
        if bool(args.draw_label):
            try:
                x, y, _, _ = cv2.boundingRect(np.vstack(p.contours))
            except Exception:
                x, y = p.bbox_xyxy[0], p.bbox_xyxy[1]
            if bool(args.display_local_xy) and p.local_xy is not None:
                label = f"P{p.instance_id} F({p.local_xy[0]:.2f},{p.local_xy[1]:.2f})"
            else:
                label = f"P{p.instance_id}"
            cv2.putText(vis, label, (x, max(18, y - 6)), cv2.FONT_HERSHEY_SIMPLEX, 0.70, color, 2, cv2.LINE_AA)
            if bool(args.draw_foot_point) and p.foot_pixel is not None:
                cv2.circle(vis, (int(p.foot_pixel[0]), int(p.foot_pixel[1])), 4, color, -1, cv2.LINE_AA)
    if bool(args.draw_mask):
...
def _flatten_config_section(data: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(data, dict):
        return {}
    sections = {"input", "hik", "video", "model", "mask", "merge", "stable_id", "local_map", "output", "display", "runtime"}
    out: Dict[str, Any] = {}
    for k, v in data.items():
        ...
        if nk in sections or nk in {"profiles", "common", "defaults", "active_profile", "notes"}:
            continue
        out[nk] = v
    for sec in ["input", "hik", "video", "model", "mask", "merge", "stable_id", "local_map", "output", "display", "runtime"]:
        obj = data.get(sec)
    ...
    try:
        while True:
            ...
            for cam_id in scan_order:
                ...
                event_item = event_item_for_frame
                if event_item is None:
                    event_item = event_overlay.capture_for_mvs_frame(
                        cam_id,
                        mvs_ts_us=ts_us,
            ...
            if frames and human_detection_enable:
                ...
                for result, meta in zip(results, metas):
                    cam_id, ts_us, fid, mvs_frame_num, mvs_meta, frame, args, event_item = meta
                    persons = extract_person_instances(result, frame.shape, args)
                    assign_local_positions(persons, projectors[cam_id], args)
