...
try:
    ...
    _ACTIVE_LED_MARKER_IMPORT_ERROR = ''
except Exception as exc:
    ActiveLedMarkerSidecar = None
    _ACTIVE_LED_MARKER_IMPORT_ERROR = f'{type(exc).__name__}: {exc}'
...
def parse_args():
    ...
    marker_options = parser.add_argument_group('Active LED identity marker options')
    marker_options.add_argument('--active-led-marker-enable', dest='active_led_marker_enable',
                                action='store_true', default=False,
                                help='Enable the default-off non-blocking active LED marker sidecar.')
    marker_options.add_argument('--disable-active-led-marker', dest='active_led_marker_enable',
                                action='store_false')
    marker_options.add_argument('--active-led-marker-roi', dest='active_led_marker_roi', type=str, default='',
                                help='Required bounded event ROI x,y,w,h when marker decoding is enabled.')
    marker_options.add_argument('--active-led-marker-output', dest='active_led_marker_output', type=str,
                                default='./sync_ipc/active_marker_observation_{camera}.jsonl')
    marker_options.add_argument('--active-led-marker-status-json', dest='active_led_marker_status_json', type=str,
                                default='./sync_ipc/active_marker_status_{camera}.json')
    marker_options.add_argument('--active-led-marker-perf-csv', dest='active_led_marker_perf_csv', type=str,
                                default='./sync_ipc/active_marker_perf_{camera}.csv')
    marker_options.add_argument('--active-led-marker-run-id', dest='active_led_marker_run_id', type=str, default='')
    marker_options.add_argument('--active-led-marker-queue-size', dest='active_led_marker_queue_size', type=int,
                                default=512)
    marker_options.add_argument('--active-led-marker-search-margin-px', dest='active_led_marker_search_margin_px',
                                type=int, default=12)
    marker_options.add_argument('--active-led-marker-stale-ms', dest='active_led_marker_stale_ms', type=float,
                                default=1000.0)
    marker_options.add_argument('--active-led-marker-alpha-ms', dest='active_led_marker_alpha_ms', type=float,
                                default=20.0)
    marker_options.add_argument('--active-led-marker-bit-count', dest='active_led_marker_bit_count', type=int,
                                default=8)
    marker_options.add_argument('--active-led-marker-checksum', dest='active_led_marker_checksum',
                                action='store_true', default=False)
    marker_options.add_argument('--active-led-marker-tolerance', dest='active_led_marker_tolerance', type=float,
                                default=0.35)
    marker_options.add_argument('--active-led-marker-bin-us', dest='active_led_marker_bin_us', type=int,
                                default=1000)
    marker_options.add_argument('--active-led-marker-min-events-per-bin',
                                dest='active_led_marker_min_events_per_bin', type=int, default=8)
    marker_options.add_argument('--active-led-marker-min-segment-events',
                                dest='active_led_marker_min_segment_events', type=int, default=20)
    marker_options.add_argument('--active-led-marker-merge-gap-us', dest='active_led_marker_merge_gap_us',
                                type=int, default=2500)
    marker_options.add_argument('--active-led-marker-polarity', dest='active_led_marker_polarity',
                                choices=['any', 'positive', 'negative', 'pos', 'neg'], default='any')
...
def main():
    ...
    marker_sidecar = None
    if bool(getattr(args, 'active_led_marker_enable', False)):
        if ActiveLedMarkerSidecar is None:
            raise RuntimeError(
                'active LED marker requested but its module could not be imported: '
                + str(_ACTIVE_LED_MARKER_IMPORT_ERROR or 'unknown import error')
            )
        marker_roi = str(getattr(args, 'active_led_marker_roi', '') or '').strip()
        if not marker_roi:
            raise ValueError('--active-led-marker-roi x,y,w,h is required when marker decoding is enabled')

        def _marker_path(raw_value, fallback):
            value = str(raw_value or fallback)
            try:
                return value.format(camera=camera_short_alias, cam=camera_short_alias,
                                    alias=camera_short_alias, serial=str(args.tsf1_camera_id))
            except Exception:
                return value

        marker_sidecar = ActiveLedMarkerSidecar(
            enabled=True,
            camera=camera_short_alias,
            camera_serial=str(getattr(args, 'tsf1_camera_id', '') or ''),
            roi=marker_roi,
            output_path=_marker_path(getattr(args, 'active_led_marker_output', ''),
                                     './sync_ipc/active_marker_observation_{camera}.jsonl'),
            status_path=_marker_path(getattr(args, 'active_led_marker_status_json', ''),
                                     './sync_ipc/active_marker_status_{camera}.json'),
            perf_csv_path=_marker_path(getattr(args, 'active_led_marker_perf_csv', ''),
                                       './sync_ipc/active_marker_perf_{camera}.csv'),
            run_id=str(getattr(args, 'active_led_marker_run_id', '') or ''),
            frame_width=int(width),
            frame_height=int(height),
            queue_size=int(getattr(args, 'active_led_marker_queue_size', 512) or 512),
            search_margin_px=int(getattr(args, 'active_led_marker_search_margin_px', 12) or 0),
            stale_after_ms=float(getattr(args, 'active_led_marker_stale_ms', 1000.0) or 1000.0),
            alpha_ms=float(getattr(args, 'active_led_marker_alpha_ms', 20.0) or 20.0),
            bit_count=int(getattr(args, 'active_led_marker_bit_count', 8) or 8),
            checksum=bool(getattr(args, 'active_led_marker_checksum', False)),
            tolerance=float(getattr(args, 'active_led_marker_tolerance', 0.35) or 0.35),
            bin_us=int(getattr(args, 'active_led_marker_bin_us', 1000) or 1000),
            min_events_per_bin=int(getattr(args, 'active_led_marker_min_events_per_bin', 8) or 8),
            min_segment_events=int(getattr(args, 'active_led_marker_min_segment_events', 20) or 20),
            merge_gap_us=int(getattr(args, 'active_led_marker_merge_gap_us', 2500) or 2500),
            polarity=str(getattr(args, 'active_led_marker_polarity', 'any') or 'any'),
        )
        print(
            f'[IR-ID][{camera_short_alias}] active LED marker sidecar enabled '
            f'roi={marker_roi} config_hash={marker_sidecar.config_hash}',
            flush=True,
        )
        ...
        try:
            ...
            event_status_writer.update(
                ...
                extra={
                    ...
                    'active_led_marker': marker_sidecar.status() if marker_sidecar is not None else {'enabled': False},
    ...
    try:
        ...
        for evs in mv_iterator:
            ...
            if marker_sidecar is not None:
                marker_sidecar.submit(
                    raw_evs,
                    int(ts),
                    int(capture_wall_time_us),
                    sync_meta={
